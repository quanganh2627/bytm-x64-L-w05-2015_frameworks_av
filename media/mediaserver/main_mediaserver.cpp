...
int main(int argc, char** argv)
{
    ...
    if (doLog && (childPid = fork()) != 0) {
        ...
        if (hlibintelwidi) {
            ALOGI("dlopen(libwidiservice) succeeded in opening from main_mediaserver.cpp.");
            ...
            if((error == NULL) && (instantiate != NULL)) {
                ...
                if(!ret) {
                    ALOGE("Could not invoke instantiate() on libwidiservice.so! Intel widi will not be used.");
                }
            }
            else {
                ALOGE("dlsym(instantiate) failed with error %s! Intel widi will not be used.", error);
            }
        }
        else {
