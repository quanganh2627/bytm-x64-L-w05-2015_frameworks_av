...
struct EffectContext {
  ...
  effect_descriptor_t*  desc;                    // Points to the sub effect
                                                 // descriptors
  audio_effect_library_t**  aeli;                // Points to the sub effect
                                                 // descriptors
  effect_handle_t       eHandle[SUB_FX_COUNT];   // The effect handles of the
                                                 // sub effects
  int32_t               index;     // The index that is currently active
                                   // - HOST or OFFLOAD
  int32_t               sessionId; // Sessiond in which the effect is created.
                                   // Stored in context to pass on to sub
                                   // effect creation
