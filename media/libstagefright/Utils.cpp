...
bool canOffloadStream(const sp<MetaData>& meta, bool hasVideo,
                      bool isStreaming, uint32_t sessionId) {
    const char *mime;
    CHECK(meta->findCString(kKeyMIMEType, &mime));
    audio_offload_info config;
    memset( &config, 0, sizeof(config) );

    config.format = AUDIO_FORMAT_INVALID;
    if (mapMimeToAudioFormat(&config.format, mime) != OK) {
        ALOGE(" Couldn't map mime type \"%s\" to a valid AudioSystem::audio_format !", mime);
        return false;
    }

    if (AUDIO_FORMAT_INVALID == config.format) {
        // can't offload if we don't know what the source format is
        ALOGE("mime type \"%s\" not a known audio format", mime);
        return false;
    }
    int32_t sampleRate;
    mapMimeToAudioFormat(&config.format, mime);
    ...
    if (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_AAC)) {
        ...
        uint32_t bitRate = -1;
        if (setAACParameters(meta, &config.format, &bitRate) != OK) {
    ...
    config.stream_type = AUDIO_STREAM_MUSIC;
    config.has_video = hasVideo;
    config.is_streaming = isStreaming;
    return AudioSystem::isOffloadSupported(config);
}
