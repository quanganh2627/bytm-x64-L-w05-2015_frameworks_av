...
sp<IRemoteDisplay> MediaPlayerService::listenForRemoteDisplay(
    ...
    if (hlibintelwidi) {
        ALOGI("dlopen(libwidiservice.so) succeeded in opening from MediaPlayerService.cpp");
        ...
        if((error == NULL) && (getRemoteDisplay != NULL)) {
            ...
            ALOGE("dlsym(getRemoteDisplay) failed with error %s. Falling back to non-Intel version.", error);
