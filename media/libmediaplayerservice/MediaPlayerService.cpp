...
MediaPlayerService::MediaPlayerService()
{
    ...
    if (hlibintelmediasink) {
        ...
        if (error == NULL && registerFactory != NULL) {
            ...
            if (!ret) {
                ALOGE("Could not invoke registerWidiRtpFactory() on libwidimediasink.so! RTP sink will not be available.");
                dlclose(hlibintelmediasink);
            }
        }
        else {
            ALOGE("dlsym(registerWidiRtpFactory) failed with error %s! RTP sink will not be available.", error);
            dlclose(hlibintelmediasink);
        }
    }
    else
