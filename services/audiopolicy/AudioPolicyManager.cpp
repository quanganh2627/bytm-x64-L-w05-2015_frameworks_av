    ...
    if (audio_is_output_device(device)) {
        ...
        case AUDIO_POLICY_DEVICE_STATE_UNAVAILABLE: {
            ...
            if (device & AUDIO_DEVICE_OUT_REMOTE_SUBMIX) {
                audio_io_handle_t BGMOutput = 0;
                AudioParameter param = AudioParameter();
                param.addInt(String8(AUDIO_PARAMETER_VALUE_REMOTE_BGM_OUTPUT),
                        BGMOutput);
                mpClientInterface->setParameters(0, param.toString());
                ALOGV("[BGMUSIC] BGM device becomes unavailable");
            }
#endif //BGM_ENABLED
                                    ...
                                    const audio_offload_info_t *offloadInfo)
{
    ...
    if (availableOutputDeviceTypes & AUDIO_DEVICE_OUT_REMOTE_SUBMIX) {
        ...
        if (param.get(String8(AUDIO_PARAMETER_KEY_REMOTE_BGM_STATE), value)
                == NO_ERROR) {
            isBGMEnabled  = (value == "true");
        }
        else {
            ALOGI("isBGMEnabled: could not get key %s",
                    AUDIO_PARAMETER_KEY_REMOTE_BGM_STATE);
            isBGMEnabled  = false;
        }
    }
    if ((isBGMEnabled) && ((stream == AUDIO_STREAM_DTMF) ||
            (stream == AUDIO_STREAM_SYSTEM) || (stream == AUDIO_STREAM_TTS) ||
        ...
        strategy = STRATEGY_BACKGROUND_MUSIC;
    } else if ((availableOutputDeviceTypes & AUDIO_DEVICE_OUT_REMOTE_SUBMIX) &&
    ...
    if (isBGMEnabled && (flags != AUDIO_OUTPUT_FLAG_REMOTE_BGM)) {
        device = getDeviceForStrategy(STRATEGY_BACKGROUND_MUSIC,
                 false /*fromCache*/);
    }
#endif //BGM_ENABLED
...
non_direct_output:
    ...
    if (audio_is_linear_pcm(format)) {
...
#ifdef BGM_ENABLED
        bool isBGMEnabled = false;
        audio_devices_t availableOutputDeviceTypes =
        ...
        if ((availableOutputDeviceTypes & AUDIO_DEVICE_OUT_REMOTE_SUBMIX) &&
            ...
            if (param.get(String8(AUDIO_PARAMETER_KEY_REMOTE_BGM_STATE),
                    value) == NO_ERROR) {
                isBGMEnabled  = (value == "true");
            } else {
                ALOGI("isBGMEnabled: could not get key %s",
        ...
        if (isBGMEnabled && (flags == 0)) {
            ...
            output = selectOutput(outputs, (audio_output_flags_t)tempflags,
                     format);
        } else {
#endif
        ...
        if ((stream == AUDIO_STREAM_MUSIC) &&
            ...
            param.addInt(String8(AUDIO_PARAMETER_VALUE_REMOTE_BGM_OUTPUT),
                    output);
            mpClientInterface->setParameters(0, param.toString());
            ALOGV("[BGMUSIC] save output %d", output);
        }
#endif
    }
                                             ...
                                             int session)
{
    ...
    if (outputDesc->mRefCount[stream] == 1) {
        ...
        if ((isBGMEnabled) && ((stream == AUDIO_STREAM_DTMF) ||
            ...
            // the remote user playback
            strategy = STRATEGY_BACKGROUND_MUSIC;
        } else if ((availableOutputDeviceTypes &
                AUDIO_DEVICE_OUT_REMOTE_SUBMIX) &&
                ((stream == AUDIO_STREAM_ALARM) ||
                        (stream == AUDIO_STREAM_NOTIFICATION))) {
            // if widi device is connected, alarm must be heard only in local,
                                            ...
                                            int session)
{
    ...
    if (outputDesc->mRefCount[stream] > 0) {
        ...
        if ((availableOutputDeviceTypes & AUDIO_DEVICE_OUT_REMOTE_SUBMIX) &&
            ...
            reply = mpClientInterface->getParameters(0,
                    String8(AUDIO_PARAMETER_VALUE_REMOTE_BGM_OUTPUT));
            param = AudioParameter(reply);
            if (param.get(String8(AUDIO_PARAMETER_VALUE_REMOTE_BGM_OUTPUT),
                    value) == NO_ERROR) {
                BGMOutput = atoi(value);
            } else {
                ALOGI("BGMOutput: could not get key %s",
                        AUDIO_PARAMETER_VALUE_REMOTE_BGM_OUTPUT);
                BGMOutput = 0;
            }
        }
        if (isBGMEnabled && (output == BGMOutput) &&
                (outputDesc->mRefCount[stream] == 0)) {
            BGMOutput = 0;
            AudioParameter param = AudioParameter();
            param.addInt(String8(AUDIO_PARAMETER_VALUE_REMOTE_BGM_OUTPUT),
                    BGMOutput);
            mpClientInterface->setParameters(0, param.toString());
            ALOGD("[BGMUSIC] stopOutput() clear background output refcount = %d",
                    outputDesc->mRefCount[stream]);
        }
#endif //BGM_ENABLED
        return NO_ERROR;
    } else {
...
void AudioPolicyManager::closeOutput(audio_io_handle_t output)
{
    ...
    if ((isBGMEnabled) && (output == BGMOutput)) {
        ...
        param1.addInt(String8(AUDIO_PARAMETER_VALUE_REMOTE_BGM_OUTPUT),
                BGMOutput);
        mpClientInterface->setParameters(0, param1.toString());
...
#ifdef BGM_ENABLED
            if ((isBGMEnabled) &&
                    (((audio_stream_type_t)i == AUDIO_STREAM_DTMF) ||
                    ((audio_stream_type_t)i == AUDIO_STREAM_SYSTEM) ||
                    ((audio_stream_type_t)i == AUDIO_STREAM_TTS) ||
                    ((audio_stream_type_t)i == AUDIO_STREAM_ENFORCED_AUDIBLE) ||
                    ((audio_stream_type_t)i == AUDIO_STREAM_RING))) {
                // if bgm is enabled, all notifications, alarm, touch tones
                // must be routed to the primary device user and it must not
                // interfere with the remote user playback
                strategy1 = STRATEGY_BACKGROUND_MUSIC;
            } else if ((availableOutputDeviceTypes &
                    AUDIO_DEVICE_OUT_REMOTE_SUBMIX) &&
                    (((audio_stream_type_t)i == AUDIO_STREAM_ALARM) ||
                            ((audio_stream_type_t)i ==
                                    AUDIO_STREAM_NOTIFICATION))) {
                // if widi device is connected, alarm must be heard only
                // in local, notification must be heard on both DUT and Widi
                // in all modes
                strategy1 = STRATEGY_SONIFICATION;
            } else {
                strategy1 = getStrategy((audio_stream_type_t)i);
            }
...
#ifdef BGM_ENABLED
    AudioPolicyManager::routing_strategy strategy;
    bool isBGMEnabled = false;
    audio_devices_t availableOutputDeviceTypes = mAvailableOutputDevices.types();
    if (availableOutputDeviceTypes & AUDIO_DEVICE_OUT_REMOTE_SUBMIX) {
        String8 reply = mpClientInterface->getParameters(0,
                                                             ...
                                                             bool fromCache)
{
    ...
    case STRATEGY_MEDIA: {
        ...
        if (isInCall() &&
            ...
            !(device & AUDIO_DEVICE_OUT_REMOTE_SUBMIX) && !(isBGMEnabled)) {
            ALOGV("%s-  superseding STRATEGY_MEDIA while in call, "
                    "follow STRATEGY_PHONE", __FUNCTION__);
                                             ...
                                             const char* address)
{
    ...
    if (!isInCall() && (isBGMEnabled) && (output != BGMOutput)) {
                                            ...
                                            audio_devices_t device)
{
    ...
    if (isBGMEnabled && (device == AUDIO_DEVICE_OUT_REMOTE_SUBMIX)) {
...
#ifdef BGM_ENABLED
    routing_strategy stream_strategy;
    if ((isBGMEnabled) && ((stream == AUDIO_STREAM_DTMF) ||
            (stream == AUDIO_STREAM_SYSTEM) ||
            (stream == AUDIO_STREAM_TTS) ||
            (stream == AUDIO_STREAM_ENFORCED_AUDIBLE) ||
        ...
        // routed to the primary device user and it must not interfere with
        // the remote user playback
        stream_strategy = STRATEGY_BACKGROUND_MUSIC;
    } else if ((availableOutputDeviceTypes & AUDIO_DEVICE_OUT_REMOTE_SUBMIX) &&
            ((stream == AUDIO_STREAM_ALARM) ||
                    (stream == AUDIO_STREAM_NOTIFICATION))) {
        // if widi device is connected, alarm must be heard only in local,
        ...
        stream_strategy = getStrategy(stream);
    }
#else
...
#ifdef BGM_ENABLED
        if ((isBGMEnabled) &&
                (((audio_stream_type_t)stream == AUDIO_STREAM_DTMF) ||
                ((audio_stream_type_t)stream == AUDIO_STREAM_SYSTEM) ||
                ((audio_stream_type_t)stream == AUDIO_STREAM_TTS) ||
                ((audio_stream_type_t)stream == AUDIO_STREAM_ENFORCED_AUDIBLE) ||
                ((audio_stream_type_t)stream == AUDIO_STREAM_RING))) {
            // if bgm is enabled, all notifications, alarm, touch tones must be
            // routed to the primary device user and it must not interfere with
            // the remote user playback
            strategy1 = STRATEGY_BACKGROUND_MUSIC;
                ...
                AUDIO_DEVICE_OUT_REMOTE_SUBMIX) &&
                (((audio_stream_type_t)stream == AUDIO_STREAM_ALARM) ||
                        ((audio_stream_type_t)stream ==
                                AUDIO_STREAM_NOTIFICATION))) {
            // if widi device is connected, alarm must be heard only in local,
            // notification must be heard on both DUT and Widi
            ...
            strategy1 = getStrategy((audio_stream_type_t)stream);
        }
        if (strategy1 == strategy) {
#else
